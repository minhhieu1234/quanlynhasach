...
class qlsach:
    ...
    def them_sach(self, sach):
        try:
            ...
            speak("<> successfully added to the bookstore <>")
        except Exception as e:
    ...
    def xoa_sach(self,ma_sach):
        try:
            with self.connection.cursor() as cursor: 
                ...
                self.connection.commit()
            speak("<> Delete book successfully <>")
        except Exception as e:
    ...
    def cap_nhat_sach(self, ma_sach, cot_thay_doi, gia_tri_moi):
        try:
            with self.connection.cursor() as cursor:
                ...
                cursor.execute(sql, val)
                self.connection.commit()
            speak("<> Update book information successfully <>")
        except Exception as e:
    ...
    def tim_kiem_sach(self, tu_khoa,cot_tim_kiem):
        try:
            with self.connection.cursor() as cursor:
                ...
                result = cursor.fetchall()
                if len(result) == 0:
                    speak("<> No books in stock <>")
                else:
                    print()
                    print(f"==================THÔNG TIN SÁCH TẠI STORE=================")
                    table = PrettyTable()
                    table.field_names = ["Mã sách", "Tên sách", "Tác giả", "Gía bán", "Số lượng"]
                    for sach in result:
                        table.add_row([sach[0], sach[1], sach[2], sach[3], sach[4]])
...
class ql_online:
    ...
    def menu(self):
        while True:
            print("="*15+ " QUẢN LÝ HÓA ĐƠN ONLINE " + "="*15)
            print("|" + " "*46 + "|")
            print("|{text:<40}|". format(text = "\t1: Lấy danh sách hóa đơn"))
            print("|{text:<40}|". format(text = "\t2: Thêm hóa đơn"))
...
class quanlynhasach:
    def menu(self):
        ...
        print("|" + " "*46 + "|")
        print("="*48)
        choice = input("Mời bạn vui lòng chọn chức năng từ (1-6) >>> ")
        if choice == "1":
            qlsach().menu()
        elif choice == "2":
            qlnv().menu_nv()
